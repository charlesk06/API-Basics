...
datenow=`date +%d-%m-%y" "%H:%M:%S`;
echo "$datenow - Loading the file into oracle AGENT_CI_TRANSACTION Table for file with Seq # $file_seq" >> $LOGPATH/interface_lftp2.log
...
/usr/bin/perl /oracle/scripts/sendsms.pl 255654876740 Tigopesa "$message";
fi
datenow=`date +%d-%m-%y" "%H:%M:%S`;
